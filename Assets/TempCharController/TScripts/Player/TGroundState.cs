...
public struct TGroundState : TICharacterState
{
    ...
    public TGroundState(TController2D controller)
    {
        if (controller == null)
        {
    ...
    private TCharacterStateData GetCharacterStateData(Vector2 movement, bool airborne)
    {
        var characterStateData = new TCharacterStateData();
        characterStateData.Movement = movement;
        if (airborne)
        {
            characterStateData.NewState = new TAirState(controller, true);
        }
        return characterStateData;
    }
    ...
    private Vector2 GetMovementDirection(Vector2 velocity)
    {
        ...
        Vector2 direction;
        var isPlanar = Vector2.Angle(Vector2.up, verticalCollisionData.SurfaceNormal) < TMathHelper.FloatEpsilon;
        if (isPlanar)
        {
    ...
    private Vector2 GetSlopeDirection(THorizontalDirection horizontalDirection, Vector2 surfaceNormal)
    {
    ...
    private void MoveAlongGround(Vector2 direction, float length, ref Vector2 movement)
    {
        ...
        var sideRayOrigin = horizontalDirection == THorizontalDirection.Right ? updatedRayCastOrigins.BottomRight : updatedRayCastOrigins.BottomLeft;
    ...
    private TCollisionData GetVerticalCollisionData(Vector2 direction, float length, TRaycastOrigins updatedRayCastOrigins)
    {
        var verticalCollisionData = new TCollisionData();
        if (direction.y > TMathHelper.FloatEpsilon)
        {
        ...
        else if (direction.y< -TMathHelper.FloatEpsilon)
        {
